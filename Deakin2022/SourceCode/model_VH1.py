...
class BottomUpVisualHead(tf.keras.layers.Layer):
    def __init__(self, num_feature=36, visual_feature_size=[32, 32]):
        ...
        self.detector = hub.load("https://tfhub.dev/google/faster_rcnn/openimages_v4/inception_resnet_v2/1").signatures[
            'default']
        ...
        if not embedding_init:
            ...
            self.embedding = tf.keras.layers.Embedding(vocab_size, emb_dim,
                                                       embeddings_initializer=tf.keras.initializers.Constant(
                                                           embedding_weights), mask_zero=True)
...
class LVAttention(tf.keras.layers.Layer):
    def __init__(self, v_dim, q_dim, num_hidden, dropout=0.3):
        ...
        self.v_dense = tf.keras.layers.Dense(num_hidden,
                                             input_shape=[None, None, v_dim], activation='relu',
                                             kernel_initializer=tf.keras.initializers.HeNormal())
        self.q_dense = tf.keras.layers.Dense(num_hidden,
                                             input_shape=[None, q_dim], activation='relu',
                                             kernel_initializer=tf.keras.initializers.HeNormal())
        self.dropout = tf.keras.layers.Dropout(rate=0.3)
        self.linear = tf.keras.layers.Dense(1, input_shape=[None, None, num_hidden],
                                            kernel_initializer=tf.keras.initializers.HeNormal())
...
class BuTdModel(tf.keras.Model):
    def __init__(self, voc_size, embedding_weights, q_dim=512, v_dim=2048, attention_hidden=1024, joint_dim=512):
        ...
        self.v_proj = tf.keras.layers.Dense(joint_dim, input_shape=[None, v_dim], activation='relu',
                                            kernel_initializer=tf.keras.initializers.HeNormal())
        self.q_proj = tf.keras.layers.Dense(joint_dim, input_shape=[None, q_dim], activation='relu',
                                            kernel_initializer=tf.keras.initializers.HeNormal())
...
class BuTdModel_Feature(tf.keras.Model):
    def __init__(self, voc_size, embedding_weights, q_dim=512, v_dim=2048, attention_hidden=1024, joint_dim=512):
        ...
        self.question_head = QuestionEmbeddingLayer(512, voc_size, 100, embedding_init=True,
